...
export async function requireSession() {
  ...
  // Voltando de um redirect OAuth (Google): a URL ainda carrega `?code=...` (PKCE) ou
  // `#access_token=...` (implícito), e o supabase-js troca isso por uma sessão em segundo plano
  // (com round-trip de rede no caso do `code`) assim que o cliente é criado — não é instantâneo.
  // Sem essa espera, esse getSession() de cima corre contra essa troca: se ela ainda não tiver
  // terminado, aqui em cima achava "sem sessão" e mandava pro login mesmo o Google tendo
  // funcionado — daí o "às vezes funciona, às vezes me manda pro login" sem padrão aparente.
  const hasOAuthError = /[?&]error=/.test(location.search) || /error=/.test(location.hash);
  const returningFromOAuth = !hasOAuthError && (/[?&]code=/.test(location.search) || /access_token=/.test(location.hash));
...
export async function loadSheet(campaignId) {
  ...
  if (campaign?.system_id) {
    ...
    attributes = stats.map((s) => ({
      ...
      formula_key: s.formula_key, default_value: s.default_value, is_resource: s.is_resource, is_skill: s.is_skill,
      is_combat: s.is_combat, category: s.category, is_hidden: s.is_hidden, color: s.color,
    }));
  } else {
...
// sistemas em destaque (fixos no topo da loja) — sem o .neq('owner_id', ...) de listStoreSystems
// de propósito: o dono do sistema em destaque também precisa vê-lo fixado quando olha a própria loja
export async function listFeaturedSystems() {
  const { data, error } = await supabase.from('systems').select('*')
    .eq('is_featured', true).eq('is_published', true).order('created_at', { ascending: false });
  if (error) throw error;
  return data;
}
