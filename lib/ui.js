...
// ============================================================
// Focus trap — prende Tab/Shift+Tab dentro de um modal aberto.
// Cada arquivo reimplementa o próprio confirmModal() (convenção do projeto: helpers duplicados
// por arquivo em vez de importados), mas o problema de foco vazando pro conteúdo atrás do modal
// é idêntico em todos — então SÓ essa parte fica centralizada aqui, e cada confirmModal() local
// chama trapFocus(w) ao abrir e a função de destroy() que ela devolve ao fechar.
// ============================================================
export function trapFocus(container) {
  const SEL = 'button, [href], input, select, textarea, [tabindex]:not([tabindex="-1"])';
  const onKeydown = (e) => {
    if (e.key !== 'Tab') return;
    const focusable = [...container.querySelectorAll(SEL)].filter((el) => !el.disabled && el.offsetParent !== null);
...
// quadro de equipamento (paper doll) com largura fixa configurada pelo Mestre (data-fit-w, em px):
// em telas mais estreitas que isso ele encolhe proporcionalmente (CSS zoom) pra caber inteiro na
// coluna, em vez de estourar a página pra direita. Slots continuam posicionados em % e o arraste do
// construtor calcula tudo em proporção do próprio retângulo do quadro, então nada disso muda de
// comportamento — só a escala visual. Auto-instalado ao importar este módulo (mesa e construtor).
function fitEqBoard(board) {
  const w = +board.dataset.fitW || 0;
  const host = board.parentElement;
  const avail = host ? host.clientWidth : 0;
  board.style.zoom = w && avail && avail < w ? String(avail / w) : '';
}
function fitAllEqBoards() { document.querySelectorAll('.eq-board[data-fit-w]').forEach(fitEqBoard); }
if (typeof window !== 'undefined' && !window.__eqFitInstalled) {
  window.__eqFitInstalled = true;
  let pending = false;
  const schedule = () => { if (pending) return; pending = true; requestAnimationFrame(() => { pending = false; fitAllEqBoards(); }); };
